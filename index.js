...
document.addEventListener('DOMContentLoaded', function () {
  // Define the repeatHandler function
  async function repeatHandler(dataText) {
    if (!sessionInfo) {
      updateStatus(statusElement, 'Please create a connection first');
      return;
    }
    ...
    const resp = await repeat(sessionInfo.session_id, dataText);
    
    updateStatus(statusElement, 'Task sent successfully');

    console.log('repeatHandler function called');
    console.log('contact data', dataText);
  }
  ...
  talkHeadings.forEach(talkHeading => {
    talkHeading.addEventListener('click', async function() {
      ...
      // Update the text content of the first '#status' element encountered
      document.querySelector('#status').textContent = `${talkHeading.textContent.trim()} ${dataText}`;
      
      // Call repeatHandler directly within the event listener
      await repeatHandler(dataText);
    });
  });
});
